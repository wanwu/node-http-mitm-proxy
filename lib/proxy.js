...
Proxy.prototype.listen = function (options, callback = (e) => {}) {
  ...
  this.httpAgent =
    typeof options.httpAgent !== 'undefined' ? options.httpAgent : new http.Agent({keepAlive: this.keepAlive});
  this.httpsAgent =
    typeof options.httpsAgent !== 'undefined' ? options.httpsAgent : new https.Agent({keepAlive: this.keepAlive});
  ...
  this.ca = options.ca || new CA(options.sslCaDir || path.resolve(process.cwd(), '.http-mitm-proxy'));
  ...
  if (self.forceSNI) {
    ...
    self._createHttpsServer({}, function (err, {port, httpsServer, wssServer}) {
      if (err) {
        return callback(err);
      }
...
Proxy.prototype._createHttpsServer = function (options, callback) {
  if (typeof options.secureOptions === 'undefined') {
    options.secureOptions = constants.SSL_OP_NO_SSLv3 || constants.SSL_OP_NO_TLSv1;
  }
  var httpsServer = https.createServer(options);
  httpsServer.timeout = this.timeout;
  httpsServer.on('error', callback);
  httpsServer.on('clientError', callback);
  ...
  function getHttpsServer(hostname, callback) {
    self.onCertificateRequired(hostname, function (err, files) {
      ...
      if (self.forceSNI && !hostname.match(/^[\d\.]+$/)) {
        ...
        try {
          self._createHttpsServer(httpsOptions, function (err, {port, httpsServer, wssServer}) {
            if (err) {
              errorHandler(e);
              return;
            }
...
Proxy.prototype._onSSLError = function (err, callback) {
  callback(err);
};
Proxy.prototype.onCertificateRequired = function (hostname, callback) {
  this.ca.getCertificateByHostname(hostname, callback);
};
...
Proxy.prototype._onWebSocketServerConnect = function (isSSL, ws, upgradeReq) {
  ...
  if (upgradeReq.url == '' || /^\//.test(upgradeReq.url)) {
    ...
    url =
      (ctx.isSSL ? 'wss' : 'ws') + '://' + hostPort.host + (hostPort.port ? ':' + hostPort.port : '') + upgradeReq.url;
  } else {
    url = upgradeReq.url;
  }
  var ptosHeaders = {};
...
Proxy.prototype._onHttpServerRequest = function (isSSL, clientToProxyRequest, proxyToClientResponse) {
  ...
  var ctx = {
    isSSL: isSSL,
    connectRequest:
      self.connectRequests[clientToProxyRequest.socket.remotePort + ':' + clientToProxyRequest.socket.localPort] || {},
